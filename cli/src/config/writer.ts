...
import { getConfigPath, getIopDir, getProgressPath } from "./loader.ts";
import type { IopConfig } from "./types.ts";

/**
 * Create the default config YAML content
 */
function createConfigContent(detected: ReturnType<typeof detectProject>): string {
	return `# iop Configuration
# https://github.com/iop-ai/sandbox
...
export function initConfig(workDir = process.cwd()): {
	...
	writeFileSync(progressPath, "# iop Progress Log\n\n", "utf-8");
...
export function addRule(rule: string, workDir = process.cwd()): void {
	...
	if (!existsSync(configPath)) {
		throw new Error(`No config found. Run 'iop --init' first.`);
	}

	const content = readFileSync(configPath, "utf-8");
	const parsed = YAML.parse(content) as IopConfig;
