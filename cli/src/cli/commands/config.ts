...
export async function showConfig(workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logWarn("No config found. Run 'iop --init' first.");
		return;
	}
	...
	console.log("");
	console.log(`${pc.bold("iop Configuration")} (${configPath})`);
	console.log("");
	...
	if (config.rules.length > 0) {
		...
		console.log(`  ${pc.dim('(none - add with: iop --add-rule "...")')}`);
	}
	console.log("");
...
export async function addRule(rule: string, workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logError("No config found. Run 'iop --init' first.");
		return;
	}
