...
export function createProgram(): Command {
	...
	program
		.name("iop")
		.description("Autonomous AI Coding Loop - Claude Code and Codex")
		.version(VERSION)
		.argument("[task]", "Single task to execute (brownfield mode)")
		.option("--init", "Initialize .iop/ configuration")
...
export function parseArgs(args: string[]): {
	...
	if (separatorIndex !== -1) {
		...
		iopArgs = args.slice(0, separatorIndex);
	}

	const program = createProgram();
	program.parse(iopArgs);
