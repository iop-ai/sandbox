import type { IopConfig } from "../config/types.ts";
...
function buildMessage(status: SessionStatus, result?: NotificationResult): string {
	if (!result) {
		return status === "completed" ? "iop session completed" : "iop session failed";
	}

	const total = result.tasksCompleted + result.tasksFailed;
	if (status === "completed") {
		return `iop session completed: ${result.tasksCompleted}/${total} tasks succeeded`;
	}
	return `iop session failed: ${result.tasksCompleted}/${total} tasks succeeded, ${result.tasksFailed} failed`;
}
