...
# iop config directory
IOP_DIR=".iop"
PROGRESS_FILE="$IOP_DIR/progress.txt"
CONFIG_FILE="$IOP_DIR/config.yaml"
SINGLE_TASK=""
INIT_MODE=false
SHOW_CONFIG=false
ADD_RULE=""
...
# ============================================
# BROWNFIELD MODE (.iop/ configuration)
# ============================================

# Initialize .iop/ directory with config files
init_iop_config() {
  if [[ -d "$IOP_DIR" ]]; then
    log_warn "$IOP_DIR already exists"
  ...
  cat > "$CONFIG_FILE" << EOF
# iop Configuration
# https://github.com/michaelshimeles/iop
...
EOF
  ...
  # Create progress.txt
  echo "# iop Progress Log" > "$PROGRESS_FILE"
  echo "" >> "$PROGRESS_FILE"

  log_success "Created $IOP_DIR/"
  echo ""
  echo "  ${CYAN}$CONFIG_FILE${RESET}   - Your rules and preferences"
  echo "  ${CYAN}$PROGRESS_FILE${RESET} - Progress log (auto-updated)"
  echo ""
  echo "${BOLD}Next steps:${RESET}"
  echo "  1. Add rules:  ${CYAN}iop --add-rule \"your rule here\"${RESET}"
  echo "  2. Or edit:    ${CYAN}$CONFIG_FILE${RESET}"
  echo "  3. Run:        ${CYAN}iop \"your task\"${RESET} or ${CYAN}iop${RESET} (with PRD.md)"
}
...
show_iop_config() {
  if [[ ! -f "$CONFIG_FILE" ]]; then
    log_warn "No config found. Run 'iop --init' first."
    exit 1
  fi

  echo ""
  echo "${BOLD}iop Configuration${RESET} ($CONFIG_FILE)"
  echo ""
  ...
  if command -v yq &>/dev/null; then
    ...
    if [[ -n "$rules" ]]; then
      ...
      echo "  ${DIM}(none - add with: iop --add-rule \"...\")${RESET}"
    fi
    echo ""
...
add_iop_rule() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    log_error "No config found. Run 'iop --init' first."
    exit 1
  fi
...
"
  ...
  # Add boundaries
  local never_touch
  never_touch=$(load_iop_boundaries "never_touch")
  if [[ -n "$never_touch" ]]; then
    prompt+="## Boundaries
...
show_help() {
  cat << EOF
${BOLD}iop${RESET} - Autonomous AI Coding Loop (v${VERSION})

${BOLD}USAGE:${RESET}
  ./iop.sh [options]              # PRD mode (requires PRD.md)
  ./iop.sh "task description"     # Single task mode (brownfield)
  ./iop.sh --init                 # Initialize .iop/ config

${BOLD}CONFIG & SETUP:${RESET}
  --init              Initialize .iop/ with smart defaults
...
${BOLD}EXAMPLES:${RESET}
  ...
  ./iop.sh --init                       # Initialize config
  ./iop.sh "add dark mode toggle"       # Run single task
  ./iop.sh "fix the login bug" --cursor # Single task with Cursor
  ./iop.sh "test the login flow" --browser  # Task with browser automation

  # PRD mode (task lists)
  ./iop.sh                              # Run with Claude Code
  ./iop.sh --codex                      # Run with Codex CLI
  ./iop.sh --branch-per-task --create-pr  # Feature branch workflow
  ./iop.sh --parallel --max-parallel 4  # Run 4 tasks concurrently
  ./iop.sh --yaml tasks.yaml            # Use YAML task file
  ./iop.sh --github owner/repo          # Fetch from GitHub issues
...
check_requirements() {
  ...
  if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
    case "$AI_ENGINE" in
      claude|cursor)
        ...
        log_info "Please run iop as a non-root user, or use a different AI engine (--opencode, --codex, --qwen, --droid, --copilot)."
        exit 1
        ;;
      *)
  ...
  if ! command -v git &>/dev/null; then
    log_error "git is required but not installed. Install git before running iop."
    exit 1
  fi

  # Check if we're in a git repository
  if ! git rev-parse --git-dir >/dev/null 2>&1; then
    log_error "Not a git repository. iop requires a git repository to track changes."
    exit 1
  fi

  # Ensure .iop/ directory exists and create progress.txt if missing
  mkdir -p "$IOP_DIR"
...
create_task_branch() {
  local task=$1
  local branch_name="iop/$(slugify "$task")"
  ...
  git stash push -m "iop-autostash" >/dev/null 2>&1 || true
  stash_after=$(git stash list -1 --format='%gd %s' 2>/dev/null || true)
  if [[ -n "$stash_after" ]] && [[ "$stash_after" != "$stash_before" ]] && [[ "$stash_after" == *"iop-autostash"* ]]; then
    stashed=true
  fi
...
create_pull_request() {
  local branch=$1
  local task=$2
  local body="${3:-Automated PR created by iop}"
...
notify_done() {
  local message="${1:-iop has completed all tasks!}"
  ...
  if command -v osascript &>/dev/null; then
    osascript -e "display notification \"$message\" with title \"iop\"" 2>/dev/null || true
  fi

  # Linux (notify-send)
  if command -v notify-send &>/dev/null; then
    notify-send "iop" "$message" 2>/dev/null || true
  fi
...
notify_error() {
  local message="${1:-iop encountered an error}"

  # macOS
  if command -v osascript &>/dev/null; then
    osascript -e "display notification \"$message\" with title \"iop - Error\"" 2>/dev/null || true
  fi

  # Linux
  if command -v notify-send &>/dev/null; then
    notify-send -u critical "iop - Error" "$message" 2>/dev/null || true
  fi
}
...
build_prompt() {
  ...
  # Add .iop/ config if available (works with PRD mode too)
  if [[ -d "$IOP_DIR" ]]; then
    # Add project context
    local context
    context=$(load_project_context)
...
run_single_task() {
  ...
  while [[ $retry_count -lt $MAX_RETRIES ]]; do
    ...
    if [[ "$CREATE_PR" == true ]] && [[ -n "$branch_name" ]]; then
      create_pull_request "$branch_name" "$current_task" "Automated implementation by iop"
    fi
...
create_agent_worktree() {
  local task_name="$1"
  local agent_num="$2"
  local branch_name="iop/agent-${agent_num}-$(slugify "$task_name")"
...
run_parallel_agent() {
  ...
  # Ensure .iop/ and progress.txt exist in worktree
  mkdir -p "$worktree_dir/$IOP_DIR"
    ...
    if [[ "$CREATE_PR" == true ]]; then
      (
        ...
        gh pr create \
          ...
          --head "$branch_name" \
          --title "$task_name" \
          --body "Automated implementation by iop (Agent $agent_num)" \
...
run_parallel_tasks() {
  ...
  for group in "${groups[@]}"; do
    ...
    if [[ "$PRD_SOURCE" == "yaml" ]] && [[ ${#group_completed_branches[@]} -gt 0 ]] && [[ ${#groups[@]} -gt 1 ]]; then
      local integration_branch="iop/integration-group-$group"
...
main() {
  ...
  if [[ -n "$SINGLE_TASK" ]]; then
    ...
    echo "${BOLD}============================================${RESET}"
    echo "${BOLD}iop${RESET} - Single Task Mode"
    ...
    if [[ -d "$IOP_DIR" ]]; then
      echo "Config: ${GREEN}$IOP_DIR/${RESET}"
    else
  ...
  # Show banner
  echo "${BOLD}============================================${RESET}"
  echo "${BOLD}iop${RESET} - Running until PRD is complete"
  local engine_display
  case "$AI_ENGINE" in
    opencode) engine_display="${CYAN}OpenCode${RESET}" ;;
    cursor) engine_display="${YELLOW}Cursor Agent${RESET}" ;;
  ...
  if [[ -d "$IOP_DIR" ]]; then
    echo "Config: ${GREEN}$IOP_DIR/${RESET} (rules loaded)"
  fi
  ...
  while true; do
    ...
    if [[ $MAX_ITERATIONS -gt 0 ]] && [[ $iteration -ge $MAX_ITERATIONS ]]; then
      log_warn "Reached max iterations ($MAX_ITERATIONS)"
      show_summary
      notify_done "iop stopped after $MAX_ITERATIONS iterations"
      exit 0
    fi
